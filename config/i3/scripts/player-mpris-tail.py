...
class Player:
    ...
    def updateIcon(self):
        self.icon = (
            ...
            ICON_PLAYING if self.status == 'paused' else
    ...
    def _parseMetadata(self):
        if self._metadata != None:
            ...
            self.metadata['artist'] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", _metadataGetFirstItem(_artist))
            self.metadata['album'] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", _metadataGetFirstItem(_album))
            ...
            self.metadata['cover'] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", _metadataGetFirstItem(_cover))
